...
class vista_catalogo(tk.Frame):
    ...
    def create_widgets(self):
        ...
        report_button = tk.Button(self.content_frame, text="Registar Producto", command=self.controller.vistaRegistroProducto)
        report_button.pack(pady=10)
    ...
    def update_catalog(self, category=None):
        ...
        # Crear la cuadrícula de productos
            for i, product in enumerate(products):
                ...
                product_name = tk.Label(product_frame, text=product[0])
                product_name.pack()

                product_category = tk.Label(product_frame, text=product[2])
                product_category.pack()

                product_price = tk.Label(product_frame, text=f"${product[4]:.2f}")
                product_price.pack()